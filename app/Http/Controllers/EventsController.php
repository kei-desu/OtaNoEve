...
class EventsController extends Controller
{
    ...
    public function backTop()
    {
        ...
        foreach( $events as $event ){
            $start_daze[] = date("Y年m月d日 H時i分", strtotime($event->start_time));
            $end_daze[] = date("Y年m月d日 H時i分", strtotime($event->end_time));
        }

        //　`Event`フォルダ内の`index`viewファイルに返す。
        // その際にview内で使用する変数を代入します。
        return view('top', compact('events', 'user', 'start_daze', 'end_daze', 'reserve', 'reserve_id'));
    }
    ...
    public function show(Event $event)
    {
        ...
        $start_daze = date("Y年m月d日 H時i分", strtotime($event->start_time));
        $end_daze = date("Y年m月d日 H時i分", strtotime($event->end_time));
        return view('event.show', compact('event', 'user', 'start_daze', 'end_daze'));
    }
